...
def create_invoice_with_autolookup(
    ...
    invoice_payload = {
        "doc_type": "proforma_invoice" if is_proforma else "tax_invoice",
        "doc_title": doc_title,
    ...
    print(f"  [SUCCESS] {doc_title} Generated ({mode_label} | {stamp_status}): {output_pdf}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Auto-Lookup Tax & Proforma Invoice Generator")
    ...
    parser.add_argument("--doc-type", choices=["tax_invoice", "proforma_invoice"], default="tax_invoice", help="Document Type: 'tax_invoice' or 'proforma_invoice'")
    parser.add_argument("--no-stamp", action="store_true", help="Generate clean invoice WITHOUT company stamp & signature")
    parser.add_argument("--output", default="outputs/autolookup_invoice.pdf", help="Output PDF Path")
